...
const Footer = () => {
  return (
    <footer className='max-container'>
        ...
        <div className='flex flex-1 justify-between lg:gap-10 gap-20 flex-wrap'>
          {footerLinks.map((section) => (
            <div key={section}>
              ...
              <ul>
                {section.links.map((link) => (<li className='mt-3 text-base leading-normal text-white-400 font-montserrat hover:text-slate-gray cursor-pointer' key={link.name}>
                  <a >{link.name}</a>
                </li>))}
              </ul>
            </div>
      ...
      <div className='flex justify-between text-white-400 max-sm:flex-col 
      mt-24 max-sm:items-center'>
        <div className='flex flex-1 justify-start font-montserrat cursor-pointer items-center gap-2'>
